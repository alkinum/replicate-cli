...
export function registerShortcuts(root: Command): void {
  ...
  imageGenerate.argument("[prompt]", "prompt").requiredOption("--model <owner/name>", "model ref").option("-o, --output <dir>", "output directory").option("--async", "return immediately").option("--wait <seconds>", "sync wait seconds", "60").option("--validate-schema", "fetch model schema and validate inputs before creating prediction").option("--dry-run", "preview request").action(
  ...
  imageEdit.requiredOption("--model <owner/name>", "model ref").requiredOption("--image <path>", "input image").option("--prompt <text>", "edit prompt").option("-o, --output <dir>", "output directory").option("--async", "return immediately").option("--validate-schema", "fetch model schema and validate inputs before creating prediction").option("--dry-run", "preview request").action(
  ...
  for (const noun of ["video", "audio"] as const) {
    ...
    command.argument("[prompt]", "prompt").requiredOption("--model <owner/name>", "model ref").option("-o, --output <dir>", "output directory").option("--async", "return immediately").option("--wait <seconds>", "sync wait seconds", noun === "video" ? "10" : "60").option("--validate-schema", "fetch model schema and validate inputs before creating prediction").option("--dry-run", "preview request").action(
      action("prediction", async (cmd, prompt?: string) => {
        const opts = cmd.opts();
        const initialBundle = await clientFor(cmd, !opts.dryRun);
        const input = await buildInput(opts, initialBundle.client, opts.dryRun);
        if (prompt) input.prompt = prompt;
        await validatePredictionInput(cmd, {
          model: opts.model,
          input,
          validateSchema: opts.validateSchema
        });
